...
#####ONLY GENEVA TRAFFIC######
Gva_Traffic = Airport_Traffic[(Airport_Traffic$airport=="Geneva"),]

#Converting to dates
Gva_Traffic = Gva_Traffic %>% mutate(date = as.Date(date, format="%Y-%m-%d"))

duplicates(Gva_Traffic)

#Adding up all the flights in the same day
Gva_Traffic = Gva_Traffic %>% aggregate(departures ~ date, sum)
head(Swiss_Traffic)

#Plotting our data
ggplot(Gva_Traffic) + geom_line(aes(x=date,y=departures))

#Quite a lot of missing data
ggplot_na_distribution(Gva_Traffic$departures)

#Using moving average to fill the missing data
Gva_Traffic$departures = na_ma(Gva_Traffic$departures)
...
Gva_F_H = left_join( x= Gva_Traffic, y=school_holidays_df, by="date")
...
```{r gva_weather_data}
#Adding data for Geneva weather
Gva_Weather = read_csv("./data/gva_weather.csv") %>% select(datetime,temp, precip, snow, windspeed, visibility, humidity)
Gva_Weather = rename(Gva_Weather, date = datetime)
...
weather <- Gva_Weather %>% mutate(Date = as.Date(date))
...
Gva_F_H_W = left_join(Gva_F_H, Gva_Weather, by = "date") 
...
ggplot_na_distribution(Gva_F_H_W$is_holiday)
ggplot_na_distribution(Gva_F_H_W$windspeed)
ggplot_na_distribution(Gva_F_H_W$departures)
ggplot_na_distribution(Gva_F_H_W$temp)
ggplot_na_distribution(Gva_F_H_W$humidity)
