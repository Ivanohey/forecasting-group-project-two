...
#STL Decomposition for the oil price
# ts_data %>%
#   model(STL(Price ~ trend(window = 7) + season(window = "periodic"),
#     robust = TRUE)) %>%
#   components() %>%
#   autoplot()
...
```{r}
plot_correlation(Gva_F_H_W)

corr_matrix <- cor(Gva_F_H_W[,2:9])
...
Gva_F_H_W <- Gva_F_H_W[, !(names(Gva_F_H_W) %in% c("snow", "visibility"))]
