...
Swiss_Traffic_tsibble %>% model(stl = STL(departures)) %>% components() %>% autoplot()
...
Holidays_ts %>% model(stl = STL(is_holiday)) %>% components() %>% autoplot()
...
ggplot(data = Flights_Holidays_Covid) +
...
#Plot the flights until COVID and holidays
ggplot(data = flights_until_covid_log) +
geom_line(aes(x = date, y = departures)) +
...
#Plot the flights until today
ggplot(data = flights_holidays_log) +
geom_line(aes(x = date, y = departures)) +
...
ggplot(data=Flights_Holidays_Covid) + geom_point(aes(y=departures, x=as.factor(is_holiday)), size=0.5, 
...
ggplot(data=balanced_data) + geom_point(aes(y=departures, x=as.factor(is_holiday)), size=0.5, position=position_jitter(width=0.05)) +
  scale_x_discrete(name = "is a holiday") +
...
cor.test(Flights_Holidays$departures, Flights_Holidays$is_holiday)
...
cor.test(Flights_Holidays_Covid$departures, Flights_Holidays_Covid$is_holiday)

rm(flights_holidays_log, flights_until_covid_log, Flights_Holidays_Covid, Holidays, Holidays_ts, weather, Weather)
```
